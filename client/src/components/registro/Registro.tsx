...
import { Grid, Paper, Avatar, Typography, TextField, Button, FormControl, FormLabel, RadioGroup, FormControlLabel, Radio, Checkbox, FormHelperText } from '@mui/material'
...
export const Registro = () => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        password: Yup
        .string().min(8, "Password minimum length should be 8").required("Required")
        .matches(/^(?=.[a-z])(?=.[A-Z])(?=.\d)(?=.[@$!%?&])[A-Za-z\d@$!%?&]{8,}$/,"Password must contain at least one uppercase letter, one number and one special character"),
