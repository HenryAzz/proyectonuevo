      ...
      {missingFilters ? (
        <Box>
          <Box sx={{ display: "flex", justifyContent: "space-around", m: 2 }}>
            ...
            <Link
              to="/logIN"
              style={{ alignSelf: "center", textDecoration: "none", color: "#B17A50" }}
            >
          ...
          {isMediumScream ? (
            <HomeMovil setStringQuery={setStringQuery} stringQuery={stringQuery} />
          ) : (
            <HomeDesktop setStringQuery={setStringQuery} stringQuery={stringQuery} />
          )}
        </Box>
      ) : (
        <FirstFilters
